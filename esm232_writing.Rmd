...
The dataframe requires daily data about minimum temperature (Celsius), and precipitations (mm). 

The variable columns of the dataset are date, day, month, year, daily precipitations,and daily minimum temperature. They need to be exactly day, month, year, tmin_c, and precip. The data is in .txt format and the following code reads it:
...
The program will run the following function for each year of data:



In the next section we will call the following function in .R format:
...
```{r}
# summarize yearly yield anomalies with bar plot

yield_plot <- ggplot(data = yield_anomalies, aes(x = year, y = yield)) +
  geom_line()+
  theme_bw() +
  labs(x = "year", 
